...
class FeedViewController: UITableViewController {
    ...
    func retrieveTweets() {
        ...
        if let account = selectedAccount {
            ...
            request.performRequestWithHandler() {
                ...
                if (error != nil) {
                    var problemFetchingTweetsAlert: UIAlertController = UIAlertController(title: "Unable to fetch tweets", message: "Oops, something went wrong.", preferredStyle: UIAlertControllerStyle.Alert)
                    var dismissButton: UIAlertAction = UIAlertAction(title: "OK", style: UIAlertActionStyle.Cancel, handler: {
                        alert in
                        self.navigationController?.popViewControllerAnimated(true)
                        return
                    })
                    
                    problemFetchingTweetsAlert.addAction(dismissButton)
                    
                    dispatch_async(dispatch_get_main_queue(), {
                        self.presentViewController(problemFetchingTweetsAlert, animated: true, completion: nil)
                    })
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let tweetData = tweets?[indexPath.row] as NSDictionary
        let userData = tweetData["user"] as NSDictionary
        
        cell.tweetContent.text? = tweetData["text"] as NSString
        cell.tweetUserName.text? = userData["name"] as NSString
